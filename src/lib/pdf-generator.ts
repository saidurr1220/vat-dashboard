...
export function generateMushok62PDF(data: Mushok62Data) {
    ...
    // Check if we need page break BEFORE adding spacing
    checkPageBreak(50); // Check if enough space for summary section

    // Add spacing after table
    yPosition += 15; // Spacing before summary section
...
export function generateMushok610PDF(data: Mushok610Data) {
    ...
    const addHeader = () => {
        ...
        // Proprietor information - same style, normal font
        doc.setFont('helvetica', 'normal');
        doc.text('Md. Saidur Rahman', 15, yPosition);
        yPosition += 5;

        doc.text('Proprietor', 15, yPosition);
        yPosition += 5;
