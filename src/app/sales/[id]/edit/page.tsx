...
export default function EditSalePage({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (saleLines.length === 0) {
      showError("No Products", "Please add at least one product to the sale");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        showSuccess("Sale Updated", "Sale has been updated successfully!");
        router.push(`/sales/${saleId}`);
      } else {
        const errorData = await response.json();
        showError("Update Failed", errorData.error || "Failed to update sale");
      }
    } catch (error) {
      console.error("Error updating sale:", error);
      showError("Update Failed", "Failed to update sale");
    } finally {
      setSubmitting(false);
    }
  };
